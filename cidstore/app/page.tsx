...
const benefits = [
  {
    ...
    description:
      "Execute demo exchanges with a smooth, lightweight flow built for speed.",
  },
  {
    title: "Secure Process",
    description:
      "Trade in a polished interface with transparent mock fees and audit-friendly summaries.",
  },
  {
    title: "Multiple Payment Methods",
    description:
      "Test QRIS, DANA, and bank transfer scenarios through one exchange experience.",
  },
  {
    title: "Customer Support",
    description:
      "Reach out through the support flow for real-time demo onboarding and follow-up.",
  },
];
...
const heroSymbols = ["BTC", "ETH", "SOL"];

export default function HomePage() {
  const { tickers, status } = useMarketData();

  const getHeroTicker = (symbol: string) => {
    ...
    return {
      symbol,
      price: fallbackCoin ? fallbackCoin.rateIdr / 16000 : 0,
      changePercent: 0,
          ...
          <div className="grid items-center gap-10 lg:grid-cols-[1.1fr_0.9fr]">
            <div>
              ...
              <p className="mt-6 max-w-xl text-lg leading-8 text-slate-300">
                Buy and sell cryptocurrency quickly with your preferred payment
                method.
              </p>
              ...
              <div className="mt-10 grid gap-4 sm:grid-cols-3">
                {[
                  ...
                  <div
                    key={stat.label}
                    className="rounded-2xl border border-white/10 bg-slate-900/70 p-4 backdrop-blur-sm"
                  >
                ...
                <div className="rounded-[28px] border border-cyan-500/20 bg-gradient-to-br from-slate-900 via-slate-900 to-slate-950 p-5">
                  ...
                  <div className="mt-6 space-y-4">
                    {heroSymbols.map((symbol) => {
                      const ticker = getHeroTicker(symbol);
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          {benefits.map((benefit) => (
            <div
              key={benefit.title}
              className="rounded-[28px] border border-white/10 bg-slate-900/80 p-5"
            >
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          {steps.map((step, index) => (
            ...
            >
              ...
              <p className="mt-3 text-sm leading-6 text-slate-300">
                {index === 0 &&
                  "Choose your preferred cryptocurrency pair from a curated list of market favorites."}

                {index === 1 &&
                  "Enter the amount and review the mock exchange summary before moving forward."}

                {index === 2 &&
                  "Select a supported payment method including QRIS, DANA, or bank transfer."}

                {index === 3 &&
                  "Confirm the order and track its demo status from the orders dashboard."}
              </p>
            </div>
          ))}
        </div>
